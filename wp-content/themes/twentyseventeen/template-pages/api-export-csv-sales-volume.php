<?php
/* Template Name: Idealhit Api Export CSV SalesVolume */
/**
 * Created by PhpStorm.
 * User: ywang
 * Date: 2018/4/25
...
if(!is_user_logged_in()){
    ...
    $action_name = 'Export CSV SalesVolume';
    $isSuccess = false;
    $msg = '';
    $csv_name = '';
    ...
    if(isset($_POST["zr"])){
        ...
        $csv_name = 'SalesVolume_' . $zr . 'days_' . date("Y.m.d_H.i.s", $erstelldatum) . '.csv';
        ...
        foreach ($product_stock_history_csv_paths as $val){
            ...
            if($helper->isDateActived($datum, $zr)){
                 ...
                 * 七天取一个文件
                 */
                $daysQuantityToToday = $helper->getDaysQuantityToToday($datum);
                if($daysQuantityToToday % 7 === 0){
                    ...
                    while(! feof($file))
                    {
                        ...
                        if($aRowInCsvArr[0] !== 'id'){
                            ...
                            $mgg = trim($aRowInCsvArr[7]);
                            $mggArr = explode('|', $mgg);
                            $mgg7days = $mggArr[3];
                            $datas['eans'][$ean][$datum] = $mgg7days;
                        }
